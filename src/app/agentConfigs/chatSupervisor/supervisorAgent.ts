...
export const getShbDocTool = tool({
  name: 'getShbDoc',
  ...
  execute: async (input, details) => {
    ...
    console.log('getShbDocTool question', question);

    const shbDoc = await getShbDoc(question);
...
export async function getShbDoc(question: string) {
  try {
